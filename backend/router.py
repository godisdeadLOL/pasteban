...
from security import validate_captcha
from utils import convert_id_to_url, decode_base64, generate_hash, generate_query, guess_language
...
def get(url: str, authorization: Optional[str] = Header(default=None), session: Session = Depends(get_session)):
    paste: Optional[Paste] = session.exec(select(Paste).where(Paste.url == url)).one_or_none()

    if paste is None:
        raise HTTPException(404)

    if paste.key_hash is not None:
        ...
        key = decode_base64(token[-1])

        if generate_hash(key) != paste.key_hash:
            raise HTTPException(401)
